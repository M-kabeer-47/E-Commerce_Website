...
const uri =
  "mongodb+srv://User1:byoabD1X2y7KzU2I@e-commerce.kopglez.mongodb.net/E-Commerce";
  ...
  try {
    ...
    if (!(await Users.exists({ email: email }))) {
      ...
      if (user.platform === "Google") {
        ...
        res.json({
          token: token,
          maxAge: 2 * 24 * 60 * 60 * 1000 + Date.now(),
          data: "Success",
        });
      } else {
...
app.get(
  ...
  function (req, res) {
    ...
    res.redirect(
      `https://e-commerce-website-cck4.vercel.app?token=${token}&maxAge=${maxAge}`
    );
  }
);
...
app.post("/addToCart/:id", authenticateUser, async (req, res) => {
  try {
    ...
    if (cart.some((item) => item.imageUrl === product.imageUrl)) {
      ...
      let productInCart = { ...product._doc, quantityInCart: quantity };
